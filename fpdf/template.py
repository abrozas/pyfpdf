...
class Template:
    def __init__(self, infile=None, elements=None, format='A4', orientation='portrait',
        ...
        self.handlers = {'T': self.text, 'L': self.line, 'I': self.image, 'A': self.write,
    ...
    def write(self, pdf, x1=0, y1=0, x2=0, y2=0, text='', font="arial", size=1, bold=False, italic=False,
              underline=False, align="", link='http://example.com', foreground=0, *args, **kwargs):
        if pdf.text_color != rgb(foreground):
            pdf.set_text_color(*rgb(foreground))
        font = font.strip().lower()
        if font == 'arial black':
        ...
        pdf.cell(w=x2-x1, h=y2-y1, txt=text, border=0, ln=0, fill=0, align=align, link=link)
